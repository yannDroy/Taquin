...
void menu (int* run, int leaderboards){
  SDL_Event event;
  int wait = 1;
  int ctrl = 0;
  ...
  while(wait){
	...
	if(leaderboards && ctrl){
	  wait = 0;
	  delete_leaderboard("leaderboard3.txt");
	  delete_leaderboard("leaderboard4.txt");
	  delete_leaderboard("leaderboard5.txt");
	  delete_leaderboard("leaderboard6.txt");
	  ctrl = 0;
	}
	break;
      case SDLK_KP3:
