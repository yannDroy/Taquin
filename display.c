...
void init (){
  ...
  test_file("leaderboard3.txt");
  test_file("leaderboard4.txt");
  test_file("leaderboard5.txt");
  test_file("leaderboard6.txt");
}
...
void init_window (SDL_Surface** window, int width, int heigth){
  *window = SDL_SetVideoMode(width, heigth, 32, SDL_HWSURFACE | SDL_DOUBLEBUF);
  ...
  if(window == NULL){
    ...
    SDL_WM_SetCaption("Taquin 3.7.2", NULL);
...
void get_player (SDL_Surface* window, char* name, int nb_moves){
  ...
  font = TTF_OpenFont("font.ttf", (window->h / 10));
