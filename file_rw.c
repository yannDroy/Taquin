...
void load_leaderboards (SDL_Surface* window, int n){
  ...
  bigfont = TTF_OpenFont("font.ttf", (window->h / 10));
...
void save_score (int n, int nfile, char* name){
  ...
  switch(nfile){
  case 3:
    file = fopen("leaderboard3.txt", "a");
    break;
  case 4:
    file = fopen("leaderboard4.txt", "a");
    break;
  case 5:
    file = fopen("leaderboard5.txt", "a");
    break;
  case 6:
    file = fopen("leaderboard6.txt", "a");
    break;
  }
